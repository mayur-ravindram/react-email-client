        ...
        <ul className="flex flex-col space-y-2 m-10">
          {data &&data.map((value, index) => (
              <Link key={index} to={`${url}/${value.id}`}>
                <li
                  key={index}
                  className={`shadow-lg p-3 rounded-sm border-l-4 ${
                    value.read ? "border-gray-400" : "border-blue-400"
                  }`}
                >
                  <div key={index} className="flex space-x-2">
                    <img
                      src={value.emailAvatarUrl}
                      alt="me"
                      height="2.5rem"
                      width="2.5rem"
                    ...
                    <div className="flex flex-col">
                      ...
                      <h1 className="font-semibold text-sm">{value.emailTo}</h1>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 font-semibold text-blue-600 flex items-center space-x-2">
                    ...
                    <span className="text-sm">
                      {moment(`${value.timestamp}`).format("dddd, MMMM DD")}
                    </span>
                    <span className="text-sm">
                      {moment(`${value.timestamp}`).format("hh:mm a")}
                    </span>
                  </div>
                </li>
              </Link>
